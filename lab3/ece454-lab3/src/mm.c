 ...
 * 
 * must be 16B aligned, therefore double word, minimum allocator must be 32B
 * 
 * use first fit
 *  
...
/* turn flag to 1 to see what is happening */
#define DEBUG 0
...
/**
 * circular doubly linked list of the free block
 * constant time removal time, add fragmeneted blocks to tail to reduce search time
 * constant tail insertion time
 */
...
/**********************************************************
 * return the next ceiling power of 2 value of some number
 **********************************************************/
unsigned return_next_pow_2(unsigned val){
    unsigned power = 1;
    while (val >>= 1) power++;
...
/**********************************************************
 * return the next ceiling power of 2 exponent of some number
 **********************************************************/
unsigned return_next_pow_2_exp(unsigned val){
    unsigned power = 1;
    while (val >>= 1) power++;
...
/**********************************************************
 * returns the index of the seg list
 **********************************************************/
int get_list_index(size_t size){
...
/**********************************************************
 * inserts (frees) block into tail of free circular linked list
 **********************************************************/
void insert_into_list(freeBlocksList * bp){
    ...
    if (segregatedListArray[index] == NULL) {
        ...
        bp->prev = segregatedListArray[index]->prev;
        bp->prev->next = bp;
        bp->next->prev = bp;
    }
#if DEBUG
...
/**********************************************************
 * removes (allocated) block from free list 
 **********************************************************/
void remove_from_list(freeBlocksList * bp){
    int index = get_list_index(GET_SIZE(HDRP(bp)));
    debugPrint("removing block %p from index %d of list\n", bp, index);
    if (bp->prev == bp){
        segregatedListArray[index] = NULL;
    }
 ...
 * 
 * initializes the segregated list with nulls
 **********************************************************/
 int mm_init(void)
 {
 ...
 * 
 * after caolesce, does not add back into free list, 
...
void * find_fit(size_t asize)
{
    ...
    for (int index = get_list_index(asize); index < NUM_SEG_LISTS; index++){
        ...
        while (bp != segregatedListArray[index]);
    }
    return NULL;
}
 ...
 * 
 * remove from the list depending on remove flag
 * 
 **********************************************************/
...
/**********************************************************
 * prints the segregated list for debugging issues
 **********************************************************/
void print_segregated_list(){
    for (int i = 0; i < NUM_SEG_LISTS; i++){
        ...
        if (bp != NULL){
            ...
            while (bp != segregatedListArray[i]);
        }
    }
}

/**********************************************************
 * prints inputs only when DEBUG flag is enabled
 **********************************************************/
void debugPrint(const char* str, ...) {
#if (DEBUG == 1)
    va_list args;
