 ...
 * use first fit
 *  
 * round up to (exclusive) 32, 64, 128, 512, 1024, 2048, 4096, 8192. beyond
...
void remove_from_list(freeBlocksList * bp){
    ...
    //previous one is the same as current, one element in linked list
    if (bp->next == bp){
...
void *coalesce(void *bp)
{
    ...
    if (prev_alloc && next_alloc) {       /* Case 1 */
        ...
        insert_into_list((freeBlocksList *) bp);
        return bp;
    }
        ...
        //re-insert into list w/ new size
        insert_into_list((freeBlocksList *) bp);
        return (bp);
    }
        ...
        insert_into_list((freeBlocksList *) PREV_BLKP(bp));
        return (PREV_BLKP(bp));
    }
        ...
        // printf("%p, size prev %lu size next %lu\n", PREV_BLKP(bp), GET_SIZE(HDRP(PREV_BLKP(bp))), GET_SIZE(FTRP(NEXT_BLKP(bp))));
        insert_into_list((freeBlocksList *) PREV_BLKP(bp));
        return (PREV_BLKP(bp));
    }
}
...
void *mm_malloc(size_t size)
{
    ...
    // printf("size: %lu, asize: %lu", size, asize);
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    //3. check if the current size is smaller than the new size
        ...
        if (coalescedSize >= asize){
            place(coalescedBlock, asize, true);
            memmove(coalescedBlock, ptr, currentSize - DSIZE);
        ...
        if (nextFitBlock != NULL){
            ...
            memmove(nextFitBlock, ptr, currentSize - DSIZE );
            return nextFitBlock;
        }
        else {
            ...
            memmove(nextFitBlock, ptr , currentSize - DSIZE );
            debugPrint("meme comp val: %d", memcmp(nextFitBlock, ptr, currentSize-DSIZE));
