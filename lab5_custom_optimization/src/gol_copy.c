...
static void
copy_board (unsigned char outboard[], const unsigned char inboard[], const int nrows, const int ncols)
{
...
static int
boards_equalp (const unsigned char b1[], const unsigned char b2[], const int nrows, const int ncols)
{
  int i;
...
main (int argc, char* argv[])
{
  ...
  /**
   * ping ponging output boards
   * each element in board contains:
   *  the curent state (1b)
   *  the neighbour count (4b)
   *  the next state (1b)
   */
  unsigned char* inboard = NULL;
  unsigned char* outboard = NULL;
  
  /**
   * changeList contains a list of pointers
   * to the cells that have been changed (ie has neighbors)
   * 
   * 
   */
  unsigned char* changeList = NULL;

  unsigned char* checkboard = NULL;
  unsigned char* final_board = NULL;
  int err = 0;
