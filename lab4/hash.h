...
template<class Ele, class Keytype> 
void 
hash<Ele,Keytype>::merge(hash * another_hash){
  unsigned i, j;

  for (i=0;i<my_size;i++){
    list<Ele, Keytype> hashed_list = another_hash->entries[i];
    Ele * hashed_element = hashed_list.pop();
    while (hashed_element != NULL){
      Ele * existing_element = lookup(hashed_element->key());
      if (existing_element){
        existing_element->count += hashed_element->count;
      }
      else {
        insert(hashed_element);
      }
      hashed_element = hashed_list.pop();
    }
  }
}
